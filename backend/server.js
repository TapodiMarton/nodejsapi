...
app.post('/savedetails/:param', function (request, response) {
  ...
  const sql = "INSERT into Students (name1, name2, email, class, birthyear) values ('" + data[0] + "', '" + data[1] + "', '" + data[2] + "', '" + data[3] + "', '" + data[4] + "')";
  console.log(sql);
  db.run(sql);
...
app.post('/update/:param', function (request, response) {
  var data = request.params.param.split(';');
  const sql = "UPDATE Students SET name1='" + data[1] + "', name2='" + data[2] + "', email='" + data[3] + "', class='" + data[4] + "', birthyear='" + data[5] + "' WHERE id='" + data[0] + "'";
  console.log(sql);
  db.run(sql);
